...
public class GameCore {
	...
	public static void Left() throws IOException
	{
		...
		for(int i = 0; i<4; i++)
		{
			...
			//Only scans if list size is bigger than 1, optimization
			if(list.size()>1)
				...
				for(int e = 0; e < list.size()-1; e++)
				{
					...
					if(value == list.get(e+1).intValue())
					{
						...
						Gui.addScore(value * 2);//Gives score to gui
					}
				}
	...
	public static void Up() throws IOException
	{
		//Puts current grid array to temp one
				...
				for(int i = 0; i<4; i++)
				{
					...
					//Iterates through list and parses to temp.grid
					for(int z = 0; z<4; z++)
					{
						temp.grid[z][i] = list.get(z);
					}
				}
